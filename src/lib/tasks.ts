  ...
  try {
    const [projects, users] = await Promise.all([
      pb.collection('projects').getFullList<
        Pick<ProjectRecord, 'id' | 'name' | 'slug' | 'status'>
      >({
        filter: 'isArchived = false || isArchived = null',
        sort: '+name',
      }),
      pb.collection('users').getFullList<
        Pick<TaskUser, 'email' | 'id' | 'name' | 'role'>
      >({
...
export function summarizeTasks(items: TaskRecord[]): TaskCollectionData['summary'] {
...
function buildTaskPayload(values: TaskFormValues, existingCompletedAt?: string) {
